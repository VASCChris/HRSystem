...
<table>
  ...
  <tbody>
    <c:forEach var='list' items="${jobList}">
			<form action='<c:url value="/management/JobControl.controller" />' method="post">
			   <tr>
				<td>${list.no}<input type="hidden" name="no" value="${list.no}"></td>
			...
			<form action='<c:url value="/management/JobControl.controller" />' method="post">
			   <tr>
				 <td>新增職務:<input type="hidden" name="no" value="0"></td>
